// Global Options
csx_opts = {
	'setupCallback': function(item){VtR1ed_setup(item);},
	'uiContainer': function(){return document;},
	'defaultFieldValue':'Edit',
...
// Master Startup
function VtR1ed_dataPostLoad(data){
...
function VtR1ed_setup(context){
	...
	//Override normalt interface changes
	VtR1ed_pips_remove_spacing();
}

// Shutdown Before Save
function VtR1ed_dataPreSave(opts){
	
	// Set additional options
	opts['context'] = document.getElementById(opts['containerId']);

	VtR1ed_unconvertBoxes(opts);
...
function VtR1ed_box(oElement,opts){
	...
	oElement.activate = function(){
		...
		if (!this.title) this.title = 'Set value';
...
function VtR1ed_convertBoxes(opts){
	...
	for (var i = 0; i < aSpans.length; i++){
		if (aSpans[i].className.match(/box/)){
			pTemp = VtR1ed_box(aSpans[i],opts);
			pTemp.showBox();
		}
	}
...
function VtR1ed_unconvertBoxes(opts) {
...
function VtR1ed_pips_remove_spacing() {
	var ancestor = document.getElementsByClassName('pipsNoSpacing')[0];
	var descendents = ancestor.childNodes;
	var i, e;
	for (i = descendents.length - 1; i >= 0; --i) {
		e = descendents[i];
		var newRight = (descendents.length - 1 - i) * 14;
